# コンテキストマップ——「インターネット美学のクロニクル」調査資料の全体地図

> 生成日: 2026-07-06
> 対象: `F:\project\internet_aesthetics` 直下の全Markdownファイル(74件)+`archive/`(反映済み文案27件・Seapunk取込文案7件)+`timeline.md`
> 用途: どのノートがどの章・どの主張の根拠か/ノート間の矛盾/未解決の【要検証】/更新が必要な箇所を一覧し、**根拠の優先順位(正本ルール)**を確定する。
> 読み落とし検証: ファイル一覧(`ls`)との突き合わせを30ファイルごとに3パスで実施(§7)。全74ファイル+archiveを分類済み。

---

## 1. 根拠の優先順位(正本ルール)

### 1-1. 資料の階層

| 階層 | 内容 | 優先順位の扱い |
|---|---|---|
| **L0 一次資料** | 各ノートが引用する公開URL(Wayback・公式カタログ・当事者ツイート等)と、[`sources/`](./sources/) 内のローカル転写・論文抽出 | 最終的な根拠。草稿に書くのは**公開URLのみ**(調査ノート§参照は書かない——next_tasks.md原則) |
| **L1 テーマ別正本ノート** | [`notes/`](./notes/) 内のフラット配置（§1-2の表で指定した「そのテーマの正」） | 事実・日付・引用の正。timeline.mdや草稿と食い違ったらまずここに戻る |
| **L2 統合資料** | `timeline.md`(日付の統合インデックス)、`README.md`(ファイル案内) | 横断参照用。**原典はL1**。timeline.mdの【要検証】V1〜V23は未解決印として維持 |
| **L3 成果物** | `草稿.md`(序文+全8章+後記) | **本文の正本**。分割稿(第N章_*.md)は手動同期の参照用で、食い違えば草稿.mdが正 |
| **L4 編集作業ファイル** | [`docs/`](./docs/) 内の計画・考察・レビュー・文案(取込_*_文案、*_計画、*_考察、レビュー系) | 意思決定の記録。事実の根拠には使わない。`archive/反映済み文案/`は履歴専用 |
| **L5 旧稿アーカイブ** | 草稿旧第2と3章.md、第3章_aestheticという語archive.md | 引用・根拠に使わない |

### 1-2. テーマ別の正本(どのノートを正とするか)

| テーマ | **正** | 補助 | 備考 |
|---|---|---|---|
| seapunk(発生・三極構造・死) | **Seapunk 詳細調査ノート.md** | supersuper.md(物理誌面)、vice_seapunkwashesup.md・nytimes_littlemermaidgoespunk.md・SilentGenerationEp22.md(転写)、ファクト補強調査#8〜10 | KYMのKevin Heckart/Pictureplane帰属は**誤り**と確定済み(正はCluster Mag 2011-12-05・Elektro Daily 2012-03-07) |
| witch house(前史・命名・派生) | **witchhouse-chillwave調査メモ.md** | Seapunk 詳細調査ノート.md §2(witch-house.comタイムライン転記) | 両者は整合。起源「地」はV3で未解決 |
| vaporwave成立・「aesthetic」語の理論 | **美学ミームの成立条件についての覚書.md** | aestheticに関する手動調査.md、[`docs/intro-kojiateki_引用メモ.md`](./docs/intro-kojiateki_引用メモ.md)(新蒸気波要点ガイド) | 覚書§3.4のKYM由来叙述は手動調査の否定的事実で**上書き**(§4-2参照) |
| 「aesthetic」の一次検証(X・Wayback・Reddit) | **aestheticに関する手動調査.md** | Google_Trends調査.md(代理指標) | 「否定的事実」(コメント欄にaesthetic無し等)はこのノートのみが持つ |
| aesthetic文法・数・モード | **aesthetic_文法分析メモ.md** | [`docs/第3章_aesthetic_rev1_考察.md`](./docs/第3章_aesthetic_rev1_考察.md)(反映記録) | 第5章rev-1の正本 |
| 「internet aesthetic」語史・Wikipedia/EBSCO制度化 | **internet_aesthetic語と制度化_調査ノート.md** | — | 序文ed-6・第5章rev-2の正本 |
| hyperpop・PC Music・brat・Drain Gang | **ハイパーポップの歴史.md** | supersuper.md §4(Cook=Roy前史)、inv-opn-cook_年表.md | Spotify Hyperpopプレイリストは**2019-08**が正(2020は誤記——ファクト補強調査で訂正済) |
| OPN×A.G. Cook交差 | **inv-opn-cook_年表.md** | — | 確度A/B/C表記あり。Guys Next DoorはB〜C=「噂として書く」 |
| Hippos in Tanks | **HipposInTanks_調査ノート.md** | MP3ブログノート§4(旧・要約版) | 重複時はHITノート優先(日付precisionが高い) |
| mp3ブログ・20jfg・GATEKEEPER・OESB・Beer on the Rug | **MP3ブログ時代とエクスペリメンタル・シーン調査ノート.md** | [`docs/src-6_仕分け.md`](./docs/src-6_仕分け.md)(取込判定) | — |
| Post T.V.・AAVV・MSV・lo-fi映像 | **witchhouse-chillwave調査メモ.md §5.4**(inv-msv) | — | ローマ映画祭公式カタログ=一次 |
| K-HOLE・normcore・VVORK・Archillect・rare.jpg・DIS人脈 | **khole-arena-archillect-researtch.md** | Interview誌オーラルヒストリー(2024-06-03、一次確定) | Brad Troemel経由・89plus委嘱・Estrada起源は**確定** |
| Are.na設計思想・CARIとの関係 | **Arena設計思想調査ノート.md** | khole-arena-archillect-researtch.md §2 | 創設年の揺れはV7(未解決) |
| CARI機関史(日付) | **CARIの歴史.md**(cari.institute/history一次) | CARI_調査ノート.md(Guardian 2016・Priz Tats・DV-i) | — |
| Aesthetics Wiki機関史 | **aesthetic wiki 歴史レポート.md** | — | Discord開設日5/16は「独自研究」と明示 |
| vaporwave政治化(fashwave・日本語圏受容) | **vaporwave政治化_調査ノート.md** | note_極右の世界のBGM.md(**二次。正文出典にしない**)、学術論文転写2本 | Rave News記事は**風刺**——実話として引用禁止(反映済み) |
| -core/-wave/-punkサフィックス | **サフィックス系譜比較調査ノート.md** | scene_subculture_notes.md(-core詳説・Scene・Indie Sleaze) | — |
| Scene・Indie Sleaze・Soft Grunge | **scene_subculture_notes.md** | — | fashioncore命名者はV15(両説併記) |
| Tumblr企業史・機能史 | **tumblrタイムライン.md**(high/mid確度付き) | MP3ブログノート§18 | — |
| Tumblrアルゴリズム(For You) | **§8-2_Tumblr_ForYou_調査メモ.md** | — | 「アルゴリズムがなかった」は2009〜2018限定でのみ妥当 |
| Web 2.0プラットフォーム通史 | Web 2.0の詳細年表.md | — | **誤記あり(V10 Instagram・V11 Vine)。他資料と矛盾時は劣後** |
| IndieWeb・Neocities・New Aesthetic・cursed images | **インディーウェブの歴史年表.md** | — | — |
| ハッシュタグ・Nanoformats | **Nanoformatsの概要と歴史.md** | — | — |
| 個別美学(dark academia・cottagecore・liminal・weirdcore・dreamcore)・発生パターンA〜F | **インターネット美学（Internet Aesthetics）調査ノート.md** | ファクト補強調査#11〜16 | liminalの多層前史(2016 Tumblrチェーン等)はこのノートが最詳 |
| 検索関心の推移 | **Google_Trends調査.md** | — | **代理指標専用**。起源・因果の根拠にしない(ノート自身のNG表) |
| lo-fi/HD区分の位置づけ | **[`docs/lofi-HD草稿全体の構造メモ.md`](./docs/lofi-HD草稿全体の構造メモ.md)** | — | 区分は**筆者による遡及的確定**であり既存用語ではない、が公式見解 |
| 匿名性の美学(人→画像) | **匿名性の美学_後半展開_論点ノート.md** | — | — |
| 日付の横断整合 | **timeline.md** | 本マップ§4 | 520項目+【要検証】V1〜V23 |

### 1-3. 矛盾時の解決手順

1. `timeline.md`の【要検証】(V1〜V23)に該当するか確認——該当すれば**両論併記のまま**(勝手に片方に確定しない)。
2. 該当しなければ§1-2の正本ノートに戻り、一次URLの確度(Wayback/公式>二次メディア>Wiki系)で判定。
3. 草稿.mdと分割稿が食い違えば草稿.mdが正。ノートと`archive/反映済み文案/`が食い違えばノートが正。
4. 確度表記を尊重: tumblrタイムライン=high/mid、inv-opn-cook=A/B/C、Seapunkノート=一次/二次、khole=▲未確認。

---

## 2. 章⇔ノート対応マップ(草稿.md 現行8章構成)

> 分割稿ファイル名は**旧6章構成の番号**のまま(第2章_命名の時代.md=現行第3章、第3章_aestheticという語.md=現行第5章、第4章_感情が先に来る.md=現行第6章、第5章_爆発.md=現行第7章、第6章_制度化.md=現行第8章)。現行第2章(シーンと土壌)・第4章(lo-fiとHD)には対応する分割稿が**ない**。

### 序文
| 主張・内容 | 根拠(正) | 反映経路 |
|---|---|---|
| 「インターネット美学」の定義・語の来歴(2020拡散→2022制度化) | internet_aesthetic語と制度化_調査ノート.md | ed-6(序文取込_ed6meta1、2026-07-01反映) |
| 「制度化」の操作的定義(命名→充填→プラットフォーム→分類体系→百科→歴史化) | 同上 §2+美学ミーム覚書 | ed-6 |
| 日本語圏の受容は射程外(カオスラウンジ等) | [`docs/編集方針.md`](./docs/編集方針.md) §2 | ed-2(反映済) |
| 執筆動機(捨て垢・布施琳太郎ツイート) | meta-1 | 2026-07-01反映 |
| 鍵概念: 命名・充填・遡行的確定 | 美学ミームの成立条件についての覚書.md §6〜7 | — |

### 第1章 土壌——Tumblrとフォークソノミー
| 主張・内容 | 根拠(正) | 補助・文案 |
|---|---|---|
| GeoCities→消滅(2009)→デジタル保存問題 | インディーウェブの歴史年表.md | 第1章取込_GeoCities土壌_文案 |
| mp3ブログ=配り選ぶ場(#Musicblogocide2k10含む) | MP3ブログノート §1 | src-6c |
| MySpace=カスタマイズされる自己(Friendster→移住・Top8) | MP3ブログノート §17+scene_subculture_notes.md §5 | src-6c |
| Tumblrアーキテクチャ(リブログ・タグ・テーマ・フォトセット) | tumblrタイムライン.md+MP3ブログノート §18 | src-6c。「アルゴリズム不在」の限定は§8-2メモで修正済 |
| フォークソノミー(ハッシュタグ却下→採用、Nanoformats) | Nanoformatsの概要と歴史.md | ファクト補強#6〜7 |
| ポストインターネットアート(語の起源・二つの流れ・Megazord) | 美学ミーム覚書 §3.1+khole §7〜8 | 第1章取込_Megazord_文案。Megazord日付群は覚書が正 |
| 日本語圏での受容(Designing Tumblr) | inv-piajp(第1章取込_piajp_文案) | README主要参照先 |
| James Bridle「新しい美学」(2011-05-06) | インディーウェブの歴史年表.md+覚書 §3.1 | — |
| Reblorg(2012-08、Tumblr公式キュレーション) | Seapunk 詳細調査ノート.md §8-2a | 第1章取込_Reblorg_文案 |
| 匿名性の見取り図(二つの流れの予告) | 匿名性の美学_後半展開_論点ノート.md | 追記1・2 |

### 第2章 シーンと土壌(2009〜2011)
| 主張・内容 | 根拠(正) | 補助・文案 |
|---|---|---|
| 骨架的インタビュー挿話(雰囲気) | intro-kojiateki_引用メモ.md(新蒸気波要点ガイド) | 第2章取込_骨架的挿話_文案 |
| マイクロジャンル見取り図 | インターネット美学ノート §4+witchhouse-chillwaveメモ §5b | — |
| OPN・Ferraroのノイズシーン離脱 | MP3ブログノート §13 | src-6d |
| witch house音響的前史(Salem・DJ Screw・Mario Zoots) | witchhouse-chillwaveメモ §2〜3+Seapunkノート §2 | 6d-review |
| 20jfg(Throbbing Gristle命名)→Tri Angle転身 | MP3ブログノート §2・6・12 | src-6b/6d |
| GATEKEEPER『Giza』VHS・Thunder Horse | MP3ブログノート §7+supersuper.md Vol2#001 | — |
| HIT・レーベルネットワーク(Games・Megazord) | HipposInTanks_調査ノート.md | src-5(第2章取込_vaporwave前史_文案) |
| Post T.V.(ローマ映画祭2010-11-04)=lo-fi映像シーンの制度的認知 | witchhouse-chillwaveメモ §5.4 | inv-msv(第2章取込_lofi映像Post-TV_文案) |
| Daniel Swan『Lux Laze』→HD転回の予告 | witchhouse-chillwaveメモ §5.4.3 | inv-swan(**未完・☐**) |
| 匿名性の美学(ペルソナ戦略・ADR証言) | MP3ブログノート §7+匿名性ノート | src-6f/rev-9 |

### 第3章 命名の時代
| 主張・内容 | 根拠(正) | 補助・文案 |
|---|---|---|
| vaporwave命名(Weed Temple 2011-10-13/Burnett) | 美学ミーム覚書 §3.2 | **V1未解決——両論併記維持** |
| chillwave=Carlesの思いつき命名(候補30語)・hypnagogic pop(Keenan) | 覚書 §3.3(一次: Hipster Runoff 2009-07-27 Wayback) | — |
| 憑在論と郷愁の言語 | witchhouse-chillwaveメモ §5b+musicplusghost.md | src-1(反映なしと確定) |
| witch house命名(Pictureplane/Shams・Last.fmタグ・コミュニティ正史) | witchhouse-chillwaveメモ §3.2+Seapunkノート §2 | src-3(r/witchhouse) |
| seapunk(ツイート→Tropicult→Coral→内紛→三極構造) | **Seapunk 詳細調査ノート.md**(全面) | Seapunk取込6a〜6f、ed-3読み道、[`docs/編集方針.md`](./docs/編集方針.md)§3(圧縮) |
| SuperSuper!=記録媒体(nu rave→witch house→seapunk→slimepunk) | supersuper.md | src-2 |
| seapunkの死(Rihanna SNL・BuzzFeed両論・Bebe Zeva) | Seapunkノート §8+aesthetic手動調査(X転記) | 6b/6c |
| seapunk→vaporwave継承(cousin記事・Blank Banshee) | Seapunkノート §8-4・10+ファクト補強#2 | 6e。Blank Banshee帰属の揺れはV23(両論併記) |
| DMY 2012-07-12/13(Harper vaporwave・Jones distroid) | 覚書 §3.2・4.1 | — |

### 第4章 lo-fiとHD
| 主張・内容 | 根拠(正) | 補助・文案 |
|---|---|---|
| lo-fi/HD区分=筆者の遡及的確定(明示) | **[`docs/lofi-HD草稿全体の構造メモ.md`](./docs/lofi-HD草稿全体の構造メモ.md)** | — |
| Eccojams(2010-08-08) vs Far Side Virtual(2011-10-25) | 覚書 §3.2+HipposInTanks_調査ノート.md | rev-8 |
| Steyerl「貧しいイメージの擁護」(2009) | inv-steyerl(第2章取込_steyerl_文案)+MP3ブログノート §14 | — |
| VVORK「無言のキュレーション」(2006〜2012) | khole-arena-archillect-researtch.md §8 | inv-khole-1a(第4章取込_VVORK_文案)+画像bot挿入_計画 |
| distroid=充填されなかった名前(DIS・#HDBOYZ・HDIY) | 覚書 §4.1+MP3ブログノート §14〜16 | — |
| conceptronica=同型の反復(2019) | 覚書 §4.3+ファクト補強#4 | — |
| PC Music(Gamsonite→DIS→SuperSuper!人脈→SOPHIE・QT) | ハイパーポップの歴史.md §1〜4+supersuper.md §4+覚書 | src-4。設立日はV8 |
| OPN×A.G. Cook交差年表 | inv-opn-cook_年表.md | 第2章取込_OPN_Cook_文案(2026-06-22) |
| deconstructed club並走(Night Slugs/Fade To Mind・Nguzunguzu) | inv-dclub文案+HITノート(Warm Pulse) | — |
| hyperpopの浮上・Spotify命名(2019-08)→brat summer | ハイパーポップの歴史.md §7・13 | src-4b |

### 第5章 「aesthetic」という語——包含関係の逆転
| 主張・内容 | 根拠(正) | 補助・文案 |
|---|---|---|
| 逆転テーゼ(vaporwave⊃aesthetic→aesthetic⊃vaporwave) | 美学ミーム覚書 §3.4 | aesthetic-reorg(章分離2026-06-19) |
| 音楽ジャンルに添えられたaesthetic(witch house 2010〜・Cluster Mag 2011-12) | aesthetic手動調査(X転記)+ファクト補強#9ブロックF | — |
| 批評の語彙になるaesthetic(Perpetua/Bebe Zeva 2012-11) | Seapunkノート §8-2a+手動調査 | 6c-1 |
| リサフランク420とコメント欄ミーム——**起源説は緩める** | **aestheticに関する手動調査.md**(否定的事実が正) | V2。KYM叙述をそのまま書かない |
| H∆SHTAG$(2013)・話者確定 | Seapunkノート §9(tumblrwave_speakers.srt) | 6c-2 |
| Savvy J「AESTHETIC」(2013-11-24)・FrankJavCee(2014-07) | 覚書 §3.4+FrankJavCee文字起こし.md | — |
| サブレディット分離(2014-10/12)・KYM項目の系列変更(2015秋) | aesthetic手動調査+覚書 §3.4(Wayback) | — |
| 文法(数・冠詞・モード)の整理 | aesthetic_文法分析メモ.md | aesthetic-rev-1 |
| 「internet aesthetic」複合語→Wikipedia AfD→復帰(2022〜2025) | internet_aesthetic語と制度化_調査ノート.md | aesthetic-rev-2 |
| CARI・Aesthetics Wikiによる上位カテゴリ固定 | 覚書 §3.5〜3.6 | — |

### 第6章 名前があとから来る(2015〜2019)
| 主張・内容 | 根拠(正) | 補助・文案 |
|---|---|---|
| サフィックスの生産性(-core/-wave/-punk) | サフィックス系譜比較調査ノート.md | inv-khole-1b(第6章_normcore追加_文案、2026-06-29反映) |
| normcore(Estrada 2008→K-HOLE 2013→NY Mag 2014横領→2024オーラルヒストリー確定) | khole-arena-archillect-researtch.md §1・7 | 同上。設立年はV5 |
| K-HOLE⇔DIS⇔89plus(委嘱確定・Troemel経由) | khole §7.1b〜1d(Interview 2024一次) | — |
| Are.na(del.icio.us動機・反アルゴリズム設計) | Arena設計思想調査ノート.md | inv-khole-1b。創設年はV7 |
| 無言のキュレーションのTwitter移植(Archillect 2014・rare.jpg 2015) | khole §3〜5・8 | 画像bot挿入_計画。**意図的な伏線**(2026-07-07)：Archillect/rare_jpgはTwitter文化の強い読者へのムードボード文化の翻訳装置であり、「著者性を消す」匿名性の美学は第7章liminal spaceの匿名性(作者不在)への伏線。Are.na→CARI(第8章)接続は現行L1246で既に明示済み |
| `## vaporwave の政治的受容`＝-waveサフィックス系列の実例として冒頭で前出し。fashwave論争(2016〜17)=synthwave/fashwaveの関係を訂正の上、コミュニティの応答として記録 | vaporwave政治化_調査ノート.md | rev-7/ed-1→6-reorg-5(⑥-full)で節全体を再構成・反映済み(2026-07-07)。Rave News=風刺の扱い遵守 |
| `### 日本語圏での受容（2018–2019・補遺）`＝木澤・現代ビジネス改題等。vaporwave政治的受容節の第三の小節（補遺）に格下げ | vaporwave政治化ノート §4 | 6-reorg-5で見出しをH3化・「（補遺）」を付与(2026-07-07) |
| 名付け親の分からない美学（dark academia・実体先行・2017-01-14投稿・COVID爆発は第7章） | インターネット美学ノート §6 | ファクト補強#13。6-reorg-1で統合節に反映済み(2026-07-07) |
| cottagecore(前史2014タグ・2018-04-25命名・WLW・Folklore) | インターネット美学ノート §7 | ファクト補強#11〜12(森ガール・Hygge・批判)。6-reorg-1で反映済み(2026-07-07) |
| Tumblr NSFW BAN(2018-12)→文化的影響力低下 | tumblrタイムライン.md | — |

### 第7章 爆発(2020〜2021)
| 主張・内容 | 根拠(正) | 補助・文案 |
|---|---|---|
| liminal spaceの多層的誕生(GSV 2008〜/cursed images 2015/Tumblrチェーン2016/4chan 2019) | インターネット美学ノート §8(最詳) | rev-10(9 Eyes)。「2019年4chan起源」通説の修正が本稿の主張 |
| The Backrooms→A24 | インターネット美学ノート §8-2 | ファクト補強#16 |
| 2020-04拡散・@elegiac_images・Solar Sands | インターネット美学ノート §8 | — |
| KYM liminal条目の編集史(2020-08創設・2021-05改稿・2023-09 Dreamcoreタイトル) | [`KYM_liminal_編集史メモ.md`](notes/KYM_liminal_編集史メモ.md) | 文案・timeline還流済み(2026-07-17)。**7-reorg-1d** |
| liminal制度化・Backrooms言説・メディア対照 | [`liminal_制度化_Backrooms_言説メモ.md`](notes/liminal_制度化_Backrooms_言説メモ.md) | **7-reorg-1e**→[`第7章_reorg_文案.md`](./archive/反映済み文案/第7章_reorg_文案.md) |
| cottagecoreの爆発(Tumblr Week in Review・Folklore 400%) | インターネット美学ノート §7 | — |
| TikTokというアーキテクチャ転換(For You対比) | §8-2_Tumblr_ForYou_調査メモ.md | 文案A+(2026-06反映) |
| -core標準化・corecore | サフィックス系譜ノート+scene_subculture_notes.md | — |
| Dreamcore/Weirdcore(anemoia) | インターネット美学ノート §8 | ファクト補強#14(DavidCrypt) |
| hyperpop第二波・glaive/midwxst | ハイパーポップの歴史.md §8 | — |
| Dismiss Yourself=Discordが音楽コミュニティになる(hex/surge・偽ジャケット) | ハイパーポップの歴史.md §9+覚書 §3.5 | ファクト補強#17(Bandcamp Daily・OTOTOY) |

### 第8章 制度化——分類するという欲望+後記
| 主張・内容 | 根拠(正) | 補助・文案 |
|---|---|---|
| Frutiger Aero=積極的な美学の生成(2017 DM命名) | CARIの歴史.md+覚書 §3.5 | — |
| CARI起源(Collins 2013 Tumblr→2014 r/Vaporwave→FBグループ) | CARIの歴史.md(一次: cari.institute/history) | cari-inv/cari-draft・ファクト補強#18 |
| Guardian 2016=vaporwaveとY2K再訪のメディア接続・Neo-Y2K(Priz Tats/DV-i) | CARI_調査ノート.md | 第5章取込_CARI_文案 |
| McBling投票(2017-01-05)・Facebook=制度化の第一層 | CARIの歴史.md+CARI_調査ノート §9 | — |
| CARIのAre.na基盤(チャンネル対応・機能分担) | Arena設計思想調査ノート.md §6〜7・10 | inv-khole-1c(第8章取込_CARI_Arena_文案)。利用開始時期はV22 |
| FairyPageとAesthetics Wiki(設立2018・400万PV・追放2021-02) | aesthetic wiki 歴史レポート.md | ファクト補強#19〜22 |
| Discordが承認プロセスになる(2020-05投票→開設) | aesthetic wiki 歴史レポート.md §5 | ed-5(r/AestheticWiki) |
| IndieWeb・Neocitiesの対位法(Bingブロック2026含む) | インディーウェブの歴史年表.md | concl-2(**未反映・☐**) |
| 後記: 歴史化=制度化(Cookプレイリスト2019-11・FairyPage・本稿のcomplicity) | ハイパーポップの歴史.md §7+[`docs/編集方針.md`](./docs/編集方針.md) §4 | ed-4(後記取込_制度化ループ_文案)・匿名性C2/C3 |
| Guardian時代診断の問いかけ | CARI_調査ノート §2 | concl-1(反映済) |

---

## 3. ノート間の矛盾(timeline.mdとの矛盾を含む)

> 一次的な対立は`timeline.md`【要検証】V1〜V23に集約済み。以下はその要約+資料構造上の矛盾。**未解決のものは両論併記を維持し、片方に確定した記述を草稿・ノートに書かない。**

### 3-1. 日付・帰属の対立(timeline.md V番号)
| # | 矛盾 | 対立する資料 | 現在の扱い |
|---|---|---|---|
| V1 | vaporwave語の初出/命名者 | 覚書(Weed Temple 2011-10-13)⇔Chicago Reader(Burnett晩秋)⇔Wikipedia | 初出(文字)と命名(帰属)を分離して両論併記 |
| V2 | 「aesthetic」YouTubeコメント欄起源説 | KYM系通説⇔aesthetic手動調査の否定的事実 | 草稿は緩め済み。**覚書§3.4のKYM叙述が未更新**(§4-2) |
| V3 | witch house起源地・時期 | Wikipedia(2006-07 NOLA/NYC)⇔witch-house.comタイムライン(2005〜分散) | 両論併記 |
| V4 | seapunk創始者 | Lil Internet側⇔Zombelle/Ultrademon側 | 「三極構造」として構造化済(Seapunkノートが正) |
| V5〜V9 | K-HOLE設立年/The Jogging開始年/Are.na創設・調達年/PC Music設立日/Neocitiesローンチ日 | 各ノート内の複数出典 | 両論併記(草稿はPC Music=6/25 infobox準拠と明記) |
| V10・V11 | Instagram開始月・Vine開始 | **Web 2.0の詳細年表.mdの誤記の可能性大** | 同ノートを劣後扱い(§1-2) |
| V12 | Guys Next Door共同名義 | Wikipedia断定⇔一次未確認 | inv-opn-cook方針=「噂として書く」 |
| V13〜V18,V20〜V23 | Atlantis対抗説/Machat死去日/fashioncore命名者/van Gennep年/Ponyboy日付/King Night月/post-internet命名/9 Eyes・Hot and Cool/CARI Are.na/Blank Banshee | 各表参照 | 両論併記または未確認と明示 |
| ~~V19~~ | ~~20jfg2008記事~~ | — | **2026-07-07 解決**（2008-10-02 THE WHITE VOID） |
| V20〜V23 | post-internet命名時期/9 Eyes×Hot and Cool/CARIのAre.na開始/Blank Banshee帰属 | 各表参照 | V23は「矛盾ではなくラベル再編成」として本文利用可 |

### 3-2. 資料構造上の食い違い(要注意)
| 食い違い | 内容 | 正 |
|---|---|---|
| **分割稿の章番号ずれ** | 第2章_命名の時代.md=草稿第3章、第3章_aestheticという語.md=草稿第5章…(旧6章構成)。現行第2章・第4章に対応分割稿なし | **草稿.md**。分割稿は参照時に番号読み替え必須 |
| 草稿_推敲メモの章番号 | 5章→6章→8章の再構成過程の番号が混在(ファイル冒頭に対応表あり) | 冒頭対応表で読み替え |
| README章構成の説明 | 現行8章と一致するが、「調査参照ファイル」表に新しいノート群が未収載(§4-1) | ファイル実体 |
| HIT記述の二重化 | MP3ブログノート§4(要約)とHITノート(詳細)が併存 | HITノート(src-6で整理済みだが§4は残存) |
| Seapunkビジュアル確立者 | 覚書§3.3「Kevin Heckartによる」⇔SeapunkノートはHeckart単独確立に慎重(Cluster Magクレジット+当事者証言まで) | Seapunkノート+ファクト補強#9の出典表 |
| Tumblr「アルゴリズム不在」 | 旧草稿の無限定過去形⇔§8-2メモ(2017 Best Stuff First以降と矛盾) | §8-2メモ(時代限定に修正済み) |
| Rave News「モントリオール緊急サミット」 | note_極右(実話扱い→追記で訂正)⇔政治化ノート(風刺と確定) | 政治化ノート。Vice(a)の日付も12/15(note_極右)ではなく**12/14**が一次 |
| Harper「加速主義のサウンドトラック」評 | 覚書は「今から見ると的外れ(hyperpopにこそ相応しい)」と評価⇔草稿はHarper引用を記録として保持 | 併存で問題なし(記録と評価の層が別)。ただし引用時は層を混同しない |

---

## 4. 【要検証】のまま未解決の箇所と理由

### 4-1. 一次資料に到達できていない(アクセス/存在の問題)
| 箇所 | ノート | 理由 |
|---|---|---|
| 4chan /x/ 2019-05-12〜13スレッド原文 | インターネット美学ノート §10-2 | 4plebsが403で確認不可 |
| CCRU×liminal spaceを接続した2015〜16のTumblrポスト | 同上 | 記憶による示唆のみ。一次資料未発見 |
| Creative Review「The New Ugly」/「Pretty Ugly」本文 | supersuper.md §7 | ペイウォール(転載ブログで代替確認済み) |
| Siepmann 2018(Psychic TV×witch house)本文 | witchhouse-chillwaveメモ | ペイウォール。要旨のみ |
| Guang Yu Weibo投稿の日付 | supersuper.md §7-9 | RCA論文脚注のURL未確認。「新丑」流通開始時期の特定に不可欠 |
| THUMP原文の「weaponized nostalgia」帰属(Franciotti本人か記者か) | vaporwave政治化ノート §3-4 | Vice 2017-01の二次報道経由のみ |
| Savvy J「AESTHETIC」原動画 | 覚書 §3.4 | チャンネルBANでロストメディア→2026-01の再投稿で内容確認可、原典は消失 |
| 骨架的インタビューの頁番号 | intro-kojiateki_引用メモ.md | 書籍確認済みだが頁番号未記録 |
| HIT014の正体(カタログ欠番) | HipposInTanks_調査ノート §9C | Discogs上でも未解決。物理盤確認か関係者照会が必要 |
| @rare_jpg運営者・「.jpg」命名慣習の起源 | khole §4 | 完全匿名・メディア露出ゼロ |
| Fandom管理画面(Aesthetics Wiki設立月日) | aesthetic wiki 歴史レポート | 403。「2018年」はthird-party情報 |

### 4-2. 検証手段が構造的に限られる(方法の問題)
| 箇所 | ノート | 理由 |
|---|---|---|
| リサフランク420コメント欄232件の網羅確認(V2) | aesthetic手動調査 | Waybackのコメント保存が2013-06-07の一部のみ。悪魔の証明化 |
| X検索による「最古の用例」全般(internet aesthetic今日的用法の初出2016〜18等) | 手動調査・internet_aesthetic語ノート | X検索は欠落・不安定(ノート冒頭に確度低と明記)。操作記録付き観察としてのみ使用可 |
| Tumblrチェーン元投稿(2016-07-15)の投稿者 | インターネット美学ノート §8 | アカウント削除済み(pez-uzi-deactivated) |
| Shamate等の独立収斂仮説 | scene_subculture_notes §7 | 直接接触の不在を示す資料が原理的に乏しい |
| Google Trendsによる因果の主張 | Google_Trends調査 | 検索≠起源。代理指標に限定(ノートのNG表が正) |

### 4-3. 調査タスクとして未着手・保留(next_tasks.md ☐)
| タスク | 内容 | 状態 |
|---|---|---|
| inv-swan | Daniel Swan(lo-fi→HD→PC Music→Ecco2k「GT-R」)の軌跡 | ☐ 並行可。Post T.V.文案で起点言及のみ |
| inv-tabor | Tabor Robakネットワーク整理・BrandNewPaintJob.exe(Jon Rafman共作) | ☐ 並行可 |
| inv-vektroid | Vektroid名義増殖 | ☐ 低優先・正文は書かない方針 |
| rev-4 | ムードボード対比(第1章) | ☐ |
| rev-12 | Caretaker×liminal/Backrooms | ☐ 先送り |
| concl-2 | HTML後方互換・アーカイブ危機・ベンダーロックイン(末尾) | ☐ 計画先行・優先度低 |
| meta-2/meta-3 | 振り返りパート/遡行的確定の自己言及(後記) | ☐ 優先度低・最後 |
| 6b残/6c残/6d-review残 | 文案メタ混入チェック・L254接続文・witch house節の分量 | ☐ 品質確認系 |
| CARI_調査ノート §8残 | Collinsギャラリー照合・トレーラー内画像特定・Terrell Davis関係 | 任意と判定済み |
| 8番出口×liminal | 第4章(現第7章)追加可否 | 判断待ち |

---

## 5. 情報が古い/更新が必要と思われる箇所

| # | 箇所 | 問題 | 推奨対応 |
|---|---|---|---|
| 1 | **README.md** | ~~「調査参照ファイル」「個別調査ノート」表に後発ノートが未収載~~ → **2026-07-07 更新済**（timeline・context-map・整合性レビュー・project-style-notes・文体分離・スキル・後発ノート追記・分割稿注意書き）。残: Web 2.0年表誤記(V10/V11)等は #4 参照 | — |
| 2 | **美学ミーム覚書 §3.4** | KYM由来の「リサフランク420→aesthetic定着」叙述が、aesthetic手動調査(2026-06-02)の否定的事実を還流していない(草稿側は修正済み、ノート側が旧) | 覚書に手動調査への相互参照と限定を追記 |
| 3 | **インターネット美学ノート §10-2(未調査リスト)** | 「-coreサフィックスの意味論」「Y2K aesthetic」「AestheticsWikiの役割」は後発ノート(サフィックス系譜・CARIの歴史・aesthetic wikiレポート・internet_aesthetic語ノート)で調査済みなのに未更新 | チェックを付け相互参照を張る |
| 4 | **Web 2.0の詳細年表** | Instagram(V10)・Vine(V11)の誤記疑い。末尾「2021年後半」の文が途切れている | 訂正または注記。timeline.mdのV10/V11を還流 |
| 5 | **tumblrタイムライン.md**(作成2026-05) | 2026-03-16リブログ抗議の「その後」(Staff対応の帰結)が未追跡 | 任意で追記 |
| 6 | **Google_Trends調査** | 2026-06-23取得のスナップショット。Trendsは再正規化されるため経時劣化する | 再取得時に差分メモ(ノート自身のTODO) |
| 7 | **分割稿6ファイル+序文.md** | 手動同期のため草稿.mdより遅れる(ノート自身が明記)。番号も旧構成 | フェーズ7(全面同期)まで参照は草稿.md優先 |
| 8 | **草稿旧第2と3章.md/第3章_aestheticという語archive.md** | 旧稿。現行の主張(例: aesthetic節の独立章化前の構成)と食い違う | 引用禁止の明示(本マップL5) |
| 9 | **_next_tasks_head.md** | next_tasks.mdの旧ヘッドコピー(重複) | 削除候補または「スナップショット」と明記 |
| 10 | **khole-arena-archillect-researtch §12** | CARIのAre.na利用(2026-06-26予備調査)の未確認事項がArena設計思想ノート(2026-06-27)で一部解消済み(Discord設立日・機能分担)だが、§12側は未更新 | Arena設計思想ノートへのポインタ追記 |
| 11 | **note_極右の世界のBGM.md** | Vice記事日付(12/15)とRave News実話扱いの二点が政治化ノートで訂正済み | 転写ファイル冒頭に「二次・訂正あり」の注記(既に政治化ノート側にはあり) |
| 12 | **ファクト補強調査「対応案」項** | Spotify hyperpop 2019訂正・ブロックFニュアンスなど「対応案」止まりの項目があり、草稿反映済みか要突合(src-4b側は反映済み表記) | 反映済みなら☑へ更新 |
| 13 | **Aesthetics Wiki現況数値** | 記事数約1,179・月間30万読者は2026-05時点 | 引用時は「時点」を必ず付す |
| 14 | **Neocities×Bing問題** | 2026-02「部分的解消」まで記録。以降の推移未追跡 | concl-2着手時に再確認 |

---

## 6. timeline.mdの位置づけ

- `timeline.md`(2026-07-06生成、本編520項目+【要検証】V1〜V23)は各ノートの日付記述の**統合インデックス**。原典は§1-2の正本ノートであり、timeline.mdの行が正本ノートと食い違う場合はノート原文(とその一次URL)を優先し、timeline.md側を修正する。
- timeline.mdの【要検証】は本マップ§3・§4と一体で管理する。解決した項目はtimeline.md本編へ昇格させ、本マップ§4から消す。
- 再生成手段: スクラッチパッドの`emit_timeline.py`+`rows_01〜24.md`(セッション一時領域のため、恒久化するならリポジトリへの移設が必要)。

---

## 7. 読み落とし検証(ファイル一覧との突き合わせ)

`ls *.md`(74件)+`archive/`に対し、30ファイルごとに3パスで全件を以下のいずれかに分類したことを確認済み(2026-07-06)。

- **パス1(1〜30件目)**: _next_tasks_head/§8-2/6d-review×2/aesthetic wiki/aesthetic_文法/aesthetic手動/Arena/CARI_調査/CARIの歴史/FrankJavCee/Google_Trends/HIT/internet_aesthetic語/intro-kojiateki/inv-opn-cook/khole/lofi-HD/MP3ブログ/musicplusghost/Nanoformats/next_tasks/note_極右/nytimes/RA×2/README/scene/Seapunk詳細/Seapunk取り込み計画 → 全て§1〜§5に分類済み
- **パス2(31〜60件目)**: SilentGeneration/src-6/supersuper/timeline/tumblrタイムライン/Unveiling/Vaporwave論文×2/vaporwave政治化/vice/Web2.0/witchhouse-chillwave/インターネット美学/インディーウェブ/サフィックス/ハイパーポップ/画像bot/序文/序文_改稿/草稿/ファクト補強/レビュー論点/推敲メモ/明晰さレビュー/草稿旧/第1章_土壌/第2章_aesthetic分離/第2章_ed3/第2章_命名/第3章_rev1/第3章_aestheticという語 → 全て分類済み
- **パス3(61〜74件目+archive)**: 第3章archive/第4章_感情/第4章取込VVORK/第5章_爆発/第6章_normcore計画・文案/第6章_制度化/第8章取込/匿名性/覚書/文章構成プラン/文体メモ/編集方針+archive/反映済み文案(27件=履歴L4)+archive/Seapunk取込文案(7件=履歴L4) → 全て分類済み

### ファイル分類総覧(役割別)

| 分類 | ファイル |
|---|---|
| 成果物(L3) | 草稿.md、timeline.md、context-map.md(本書) |
| 分割稿・同期物(L3参照用) | 序文.md、第1章_土壌.md、第2章_命名の時代.md、第3章_aestheticという語.md、第4章_感情が先に来る.md、第5章_爆発.md、第6章_制度化.md |
| テーマ別正本ノート(L1) | [`notes/`](./notes/)（§1-2の表・29ファイル・フラット） |
| 一次資料ローカル転写(L0) | [`sources/transcripts/`](./sources/transcripts/)・[`sources/papers/`](./sources/papers/)（vice/nytimes/SilentGeneration/FrankJavCee/musicplusghost/note_極右、RA×2、Vaporwave論文×2、Unveiling_Tumblr's 等） |
| 統合・案内(L2) | README.md、[`docs/project-style-notes.md`](./docs/project-style-notes.md)、[`docs/文体メモ.md`](./docs/文体メモ.md) |
| メタ・編集(L4/L5) | [`docs/`](./docs/)（計画・考察・レビュー・文体・編集方針・旧稿）。`next_tasks.md`・`_next_tasks_head.md`はルート |
| 計画・考察(L4) | [`docs/`](./docs/)内：文章構成プラン、編集方針、src-6_仕分け、Seapunk調査_取り込み計画、第2章_ed3計画、第2章_aesthetic章分離_考察、第3章_aesthetic_rev1_考察、6d-review×2、lofi-HD構造メモ、画像bot挿入_計画、第6章_normcore追加_計画、第6章/第7章_reorg_計画 |
| レビュー(L4) | [`docs/`](./docs/)内：草稿_推敲メモ、草稿_明晰さレビュー、草稿_レビュー論点メモ、**草稿_整合性レビュー**、草稿_ファクト補強調査(※事実訂正を含むためL1に準ずる) |
| Claude Code スキル | `.claude/skills/manuscript-style/SKILL.md`（文体メモの実行用版） |
| 文案(L4・反映済みはarchiveが原則) | archive/反映済み文案31件+archive/Seapunk取込文案7件（ルート残置なし・2026-08-04整理） |
| 旧稿(L5) | [`docs/草稿旧第2と3章.md`](./docs/草稿旧第2と3章.md)、[`docs/第3章_aestheticという語archive.md`](./docs/第3章_aestheticという語archive.md)、序文_改稿.md(参照メモ化済み) |
